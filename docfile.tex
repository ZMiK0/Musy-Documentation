...
closeDropDown() {
    this.isDropDownOpen = false;
                ...
                Para acceder a las opciones de las canciones un \textit{dropdown menu} es desplegado.
...
addSongToQueue() {
    ...
    this.songManagement.addOneSong(song);
                ...
                Los metadatos de la canción son empaquetados en una variable de tipo Song para posteriormente ser enviados al servicio \verb|song-management| ya sea para su inmediata reproducción o para ser añadidos a la cola de canciones.
...
close() {
    ...
    this.songAdding.letGo();
                ...
                Se reciben y se almacenan todas las playlist en el servicio \verb|song-adding| y se abre el modal con la lista de playlist para elegir en cual insertar la canción.

                Una vez la canción ha sido añadida se cierra el menu modal y se vuelve a llamar al servicio para que libere todas las playlist de su memoria y asi mejorar el rendimiento.
...
async getCoverPath(): Promise<string> {
    ...
    return URL.createObjectURL(blob);
                ...
                Mismo método usado en el componente anterior.
